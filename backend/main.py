...
# Define columns for hourly intervals
time_columns = [
...
def get_years():
    # Fetch distinct years from the 'time_series_data' collection
    years = collection.distinct("Date")
    distinct_years = sorted(set(int(date.split('-')[2]) for date in years))  # Extract year and convert to int
    return jsonify(distinct_years)

@app.route('/months/<int:year>', methods=['GET'])
def get_months(year):
    # Fetch distinct months for the given year
    months = collection.distinct("Date")
    distinct_months = sorted(set(int(date.split('-')[1]) for date in months if int(date.split('-')[2]) == year))  # Extract month
    return jsonify(distinct_months)
